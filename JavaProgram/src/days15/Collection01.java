...
// - 크기가 고정, 데이터 중간삽의과 중간삭제에 비효율적

// 2. 리스트(List)
// - 크기의 제약없이 데이터를 저장할 수 있음, 데이터의 삽입과 삭제에 최적화
// - 검색에 취약 - (참조값(주소)을 저장하기 때문) - 이를 개선하기 위한 더블 링크드 리스트를 사용하기도 함
//- 데이터 저장시 불필요한 메모리 사용
...
public class Collection01 {
	...
	public static void main(String[] args) {
		...
		//v.get(i)를 %d에 적용할때는 강제 캐스팅이 필요 없습니다.
		//Vector,ArrayList 클래스의 get메소드 :특정 인덱스에 저장되어 있는 데이터를 반환
		System.out.println();
		for(int i=0;i<a.size();i++)
			System.out.printf("a[%d]=%d\t\t",i,a.get(i));
		
		//인덱스 매개변수를 추가하여 데이터 삽입 가능.
