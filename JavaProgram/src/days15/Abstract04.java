...
// 추상 메소드의 갯수가 많은면 상속에 부담을 주게 됩니다.
// 자식 클래스에서 사용하지 않을 추상 메소드라도, 객체 생성을 위해 반드시 구현(overriding)해야하는 문제점이 있습니다.
...
//Abstract_B 클래스를 상속 받은 클래스는 필요없는 메서드가 있어도, 추상 메서드를 모두 오버라이드(구현)해야 합니다.
...
// 이 추상 클래스를 예상컨데 자주 사용할 예정이라면, 모든 추상메서드가 구현(오버라이드) Adapter클래스를 생성하고,
