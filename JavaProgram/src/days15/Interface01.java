...
// 1. 인터페이스도 클래스 입니다. 다만 제약이 있는 클래스 입니다.
...
public class Interface01 {
	...
	public static void main(String[] args) {
		// InterA a = new InterA(); 
		// 에러 - 추상 메서드가 있는 클래스는 객체를 만들수 없습니다.
		// 인터페이스에는 생성자가 없습니다.
		// 추상 클래스에는 생성자가 존재합니다. 그러나 추상 메소드 때문에 객체생성은 안됩니다.
		
		// 인터페이스는 상속을 통한 다형성 구현엣만 인터페이스가 활용돔
		// 부모 인터페이스의 레퍼런스를 사용하여 자식 클래스의 객체를 참조 가능
		InterA a1 =new SubA();
		a1.test();	//부모 참조변수로 오버라이딩 된 메서드 실행
		
		SubA a2 = new SubA();
		method1(a2);
	}

	private static void method1(InterA a) { // 매개변수로 부모 참조변수 사용
		a.test();
	}
