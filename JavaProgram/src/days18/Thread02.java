...
// 구현 방법
//1. Thread 클래스를 상속받아 public void run() 메서드를 오버라이딩 합니다.
//	 public void run() : 쓰레드가 생성되어 수행할 작업을 정의하는 메서드.
//1-2. 해당 클래스의 객체를 생성하고, start 메서드를 호출(실행)합니다.
// 		만약 오버라이딩 된 run메서드를 호출하면, 쓰레드 생성실행이 아니라, 일반 메서드 호출이
//		되므로 반드시 start 메서드를 통해 run으로 이어지게 호출합니다.
// 	 start 메서드 : Thread 클래스(부모클래스)에 있는 메서드로, 자체적으로 쓰레드 추가
//						생성 후 run 메서드를 재호출하는 역할을 합니다.(상속된 메서드로 메서드가 표면에 보여지지 않은채 사용합니다.)
	...
	public void run() {
		for(int i=1; i<=10;i++) {
			...
			try {
				//Thread.sleep(300);
				sleep(300);
				//sleep메서드는 Thread 클래스에 있는 static 메서드 입니다.
...
public class Thread02 {
	...
	public static void main(String[] args) {
		...
		//b1.run();
		//b2.run();		//run 메서드를 직접 호출하는 것은 쓰레드 생성없이 단순실행을 시키는 것이므로 앞선 예제와 같은 결과가 나옵니다.
		
		b1.start(); 	//별도의 쓰레스 생성&실행
		b2.start();	//별도의 쓰레스 생성&실행
