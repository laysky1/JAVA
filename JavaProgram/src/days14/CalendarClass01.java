...
import java.util.Calendar;
	...
	class CalendarClass01 {

	public static void main(String[] args) {
		
		Calendar today = Calendar.getInstance(); //현재날짜는 오늘 날짜
		// 싱글톤 방식의 클래스
		// 생성자가 private으로 접근 차단 됨(외부에서 접근 금지 -> 외부에서 new Calendar() 사용 안됨)
		// 유일하게 실행된 new Calendar()는 클래스 내부에서 실행되어 클래스내부에 단 하나만 존재합니다.
		...
		Calendar tommorrow= Calendar.getInstance(); // 특별히 날짜지정을 하지 않으면 현재 날짜는 오늘 날짜
		Calendar yesterday=Calendar.getInstance();
		// 공유된 인스턴스에서 현재날짜에 관한 정보만 객체까지 서로 다른 날짜를 따로 관리하고 많은 내용이 공유됩니다.
		
		//캘릭더 클래스에는 스태틱 파이널(static final)변수들이 다수 있습니다.
		...
		// 연도에 필요한 전달인수 1 대신에 Calendar.YEAR를 사용해서 연도 추출
		System.out.println("년도 : "+today.get(Calendar.YEAR));
